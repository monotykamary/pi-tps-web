  ...
  const CustomTooltip = ({ active, payload }: any) => {
    ...
    return (
      <div className="glass-panel rounded-xl px-3 py-2 text-xs shadow-diffuse">
        <span className="font-semibold text-slate-700">{d.name}:</span> <span className="metric-mono">{d.value.toLocaleString()}</span>
      </div>
    );
  };
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-5">
        ...
        <h2 className="text-base font-semibold tracking-tight text-slate-800">Cache Efficiency</h2>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="relative">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
            <span className="metric-mono text-2xl font-bold text-slate-800">{cacheHitRate.toFixed(0)}%</span>
            <span className="text-[10px] text-slate-400 font-medium uppercase tracking-wider">cache hit</span>
          </div>
        </div>
          ...
          {overall.map(item => (
            <div key={item.name}>
              <div className="flex items-center justify-between text-xs mb-1">
                ...
                <span className="metric-mono text-slate-700 font-semibold">{((item.value / overall.reduce((s, v) => s + v.value, 0)) * 100).toFixed(0)}%</span>
              </div>
      ...
      <div className="mt-5 pt-4 border-t border-slate-100">
        <p className="text-[10px] font-semibold uppercase tracking-wider text-slate-400 mb-3">Cache hit rate over time (by request range)</p>
            ...
            return cacheOverTime.map((c, i) => {
              ...
              return (
                <div key={i} className="flex-1 flex flex-col items-center gap-1">
                  ...
                  />
                  <span className="text-[9px] metric-mono text-slate-400">{c.hitRate}%</span>
                </div>
              );
            });
          })()}
